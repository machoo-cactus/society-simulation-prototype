...
from stage0_sim.adapters.llm.fake import FakeDialogueGenerator, ScriptedPlanner
            ...
            "entities": [
                {
                    ...
                    "components": {
                        ...
                        "planner": {
                            "daily_goals": ["Visit the lounge"],
                            "current_priorities": ["Choose a route"],
                        },
...
def test_tool_decision_capture_persists_rounds_choices_and_action_outcome(
    ...
    client = ScriptedModelClient(
        (
            ...
            _model_turn("go_to", {"target_id": "lounge"}),
        )
    )
    ...
    assert request["observation"]["goals"] == ["Visit the lounge"]
    assert request["observation"]["current_priorities"] == [
        "Choose a route"
    ]
    ...
    assert decision[0:2] == ("completed", "tool:go_to")
...
def test_planner_dialogue_and_memory_provider_boundaries_are_private(
    tmp_path: Path,
) -> None:
    database = tmp_path / "private-macro.sqlite3"
            ...
            "entities": [
                {
                    ...
                    "components": {
                        ...
                        "memory": {
                            "initial_episodes": [
                                {
                                    "text": "A private prior conversation.",
                                    "importance": 0.8,
                                }
                            ]
                        },
    ...
    record_types = {
        ...
        for row in connection.execute(
            ...
            WHERE run_id = ? AND visibility = 'PRIVATE_RESEARCH'
            """,
            ("private-macro",),
        )
    }
    ...
    memory_subjects = {
        ...
        for row in connection.execute(
        """
        SELECT subject_id FROM memory_relations
        WHERE run_id = ? AND relation_type = 'source'
        """,
        ("private-macro",),
        )
    }
    assert {"speaker", "listener"} <= memory_subjects
    interaction = connection.execute(
        """
        SELECT status, episode_json FROM interaction_episodes
        WHERE run_id = ? AND interaction_type = 'generated_dialogue'
        """,
        ("private-macro",),
    ).fetchone()
    assert interaction[0] == "completed"
    participants = json.loads(interaction[1])["participants"]
    assert {
        (participant["participant_id"], participant["role"])
        for participant in participants
    } == {
