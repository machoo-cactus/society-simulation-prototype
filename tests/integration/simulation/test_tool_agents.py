...
def test_scripted_controller_moves_only_through_tool_commit() -> None:
    client = ScriptedModelClient(
        (_turn("navigate_to", {"target_id": "lounge", "reason": "Take a break"}),)
    )
...
def test_tool_registry_rejects_extra_fields_and_unknown_targets() -> None:
    ...
    request = CharacterDecisionRequest(
        ...
        memories=(),
        allowed_tools=("navigate_to", "wait"),
    )
    ...
    with pytest.raises(ToolValidationError) as target:
        registry.propose(
            request,
            ModelToolCall("call-2", "navigate_to", {"target_id": "hidden"}),
        )
...
def test_navigate_tool_produces_typed_cross_building_intent() -> None:
    observation = CharacterObservation(
        agent_id="alex",
        display_name="Alex",
        simulation_time=0,
    ...
    assert intent.kind.value == "navigate"
    assert intent.target_id == "building-office"
    assert intent.preferred_mode.value == "CAR"
...
def _two_character_tool_scenario() -> ScenarioDefinition:
    ...
    second["components"]["goals"]["goals"][0]["id"] = "blair-take-a-break"
    payload["entities"].append(second)
    return ScenarioDefinition.model_validate(payload)


def test_removed_cognition_modes_and_controller_choices_are_rejected() -> None:
    payload = _tool_scenario().model_dump(mode="json")
    payload["cognition"]["execution_mode"] = "background"
    with pytest.raises(ValueError, match="execution_mode"):
        ScenarioDefinition.model_validate(payload)
    payload["cognition"].pop("execution_mode")
    payload["cognition"]["controller"] = "legacy"
