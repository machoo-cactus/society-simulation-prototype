"""ECS component definitions."""

from stage0_sim.domain.components.affordance import (
    AffordanceExecutionComponent,
    AffordanceRequestComponent,
)
from stage0_sim.domain.components.cognition import (
    CharacterProfileComponent,
    CharacterSituationComponent,
    ControllerComponent,
    ConversationComponent,
)
from stage0_sim.domain.components.content import (
    ContentAccessMode,
    ContentEndpoint,
    ContentEndpointComponent,
    ContentEndpointKind,
    KnownTextAddressesComponent,
)
from stage0_sim.domain.components.effects import (
    CarriedLoadComponent,
    CharacterEmbodimentComponent,
    EffectOperation,
    EquipmentSlot,
    EquipmentStateComponent,
    ObjectDimensions,
    ObjectEffect,
    ObjectIntrinsicComponent,
    ObjectSizeClass,
    ScentSourceComponent,
    SenseEffectTarget,
    WearableComponent,
)
from stage0_sim.domain.components.engagement import (
    EngagementArgument,
    EngagementCapabilityInvocation,
    EngagementExecutionComponent,
    EngagementGroupExecution,
    EngagementGroupStatus,
    EngagementInvocationGroup,
    EngagementProgram,
    EngagementProgramComponent,
    EngagementStatus,
    EngagementValidationIssue,
    PendingEngagementComponent,
    RejectedEngagementGroup,
)
from stage0_sim.domain.components.goals import (
    ActionOutcome,
    ActionOutcomeCriterion,
    EventMatchCriterion,
    GoalComparator,
    GoalCompletionPolicy,
    GoalComponent,
    GoalCriterion,
    GoalCriterionEffect,
    GoalDefinition,
    GoalEvidence,
    GoalLocationKind,
    GoalRuntime,
    GoalStateComponent,
    GoalStatus,
    InteractionCountCriterion,
    InteractionType,
    LocationMatchCriterion,
    PossessionThresholdCriterion,
    SimulationTimeCriterion,
    StateComparisonCriterion,
)
from stage0_sim.domain.components.information import InformationNamespaceComponent
from stage0_sim.domain.components.interaction import (
    InteractionExecutionComponent,
    InteractionRequestComponent,
)
from stage0_sim.domain.components.memory import MemoryComponent
from stage0_sim.domain.components.navigation import (
    NavigationComponent,
    NavigationPrimitive,
    NavigationPrimitiveKind,
    NavigationStatus,
)
from stage0_sim.domain.components.npcs import NpcComponent
from stage0_sim.domain.components.perception import (
    EffectiveSensesComponent,
    KnowledgeRecord,
    PerceptionComponent,
    SensesComponent,
)
from stage0_sim.domain.components.physical import (
    STANDING_CHARACTER_FOOTPRINT,
    CardinalOrientation,
    CharacterHandStateComponent,
    CharacterPosture,
    CharacterPostureComponent,
    ConsumableComponent,
    ContainerComponent,
    CustodyComponent,
    Footprint,
    LegacyCellCoordinate,
    MicrocellCoordinate,
    MovementObstruction,
    OccupancySlot,
    OccupancySlotsComponent,
    OpenableComponent,
    OwnershipComponent,
    PhysicalInteractionRegistry,
    PhysicalInteractionTarget,
    PhysicalObjectIdentityComponent,
    PhysicalPose,
    PhysicalRelationKind,
    PhysicalStateComponent,
    PortableComponent,
    ReadableComponent,
    SenseModality,
    SenseTransmission,
    SpatialCollisionError,
    SpatialIndex,
    SpatialIndexEntry,
    SpatialMetric,
    SpatialParentRelationComponent,
    SupportComponent,
    UsableComponent,
    VisionObstruction,
    footprints_touch,
    validate_spatial_relation_acyclicity,
)
from stage0_sim.domain.components.physiology import (
    ActivityComponent,
    ActivityRates,
    ActivityType,
    HomeostasisComponent,
    HomeostasisConfiguration,
    default_activity_rates,
)
from stage0_sim.domain.components.planning import (
    ActionGoalLink,
    ActionInstance,
    ActionOrigin,
    ActionType,
    GoalLinkKind,
    LineageIdGenerator,
    PlanAction,
    PlanComponent,
)
from stage0_sim.domain.components.possessions import (
    PossessionsComponent,
    TransactionExecutionComponent,
    TransactionRequestComponent,
)
from stage0_sim.domain.components.spatial import MovementComponent, PositionComponent
from stage0_sim.domain.components.speech import PendingSpeechComponent
from stage0_sim.domain.components.survival import (
    DriveComponent,
    DriveThreshold,
    DriveType,
    System1Configuration,
    System1State,
    default_drive_thresholds,
)
from stage0_sim.domain.components.text_action import (
    PendingTextReceiptsComponent,
    TextActionExecutionComponent,
    TextActionRequestComponent,
    TextContentPersistenceBinding,
)
from stage0_sim.domain.components.travel import (
    SpatialLocationComponent,
    TravelComponent,
)
from stage0_sim.domain.interactions import (
    InteractionFailureReason,
    InteractionSpecification,
    InteractionVerb,
)

__all__ = [
    "ActionType",
    "ActionGoalLink",
    "ActionInstance",
    "ActionOrigin",
    "AffordanceExecutionComponent",
    "AffordanceRequestComponent",
    "ActivityComponent",
    "ActivityRates",
    "ActivityType",
    "CharacterProfileComponent",
    "CarriedLoadComponent",
    "CharacterEmbodimentComponent",
    "CharacterHandStateComponent",
    "CharacterPosture",
    "CharacterPostureComponent",
    "CharacterSituationComponent",
    "ConversationComponent",
    "ContentAccessMode",
    "ContentEndpoint",
    "ContentEndpointComponent",
    "ContentEndpointKind",
    "ConsumableComponent",
    "ContainerComponent",
    "ControllerComponent",
    "CustodyComponent",
    "DriveComponent",
    "DriveThreshold",
    "DriveType",
    "EngagementArgument",
    "EngagementCapabilityInvocation",
    "EngagementExecutionComponent",
    "EngagementGroupExecution",
    "EngagementGroupStatus",
    "EngagementInvocationGroup",
    "EngagementProgram",
    "EngagementProgramComponent",
    "EngagementStatus",
    "EngagementValidationIssue",
    "EffectOperation",
    "EffectiveSensesComponent",
    "EquipmentSlot",
    "EquipmentStateComponent",
    "EventMatchCriterion",
    "GoalComparator",
    "GoalCompletionPolicy",
    "GoalComponent",
    "GoalCriterion",
    "GoalCriterionEffect",
    "GoalDefinition",
    "GoalEvidence",
    "GoalLocationKind",
    "GoalLinkKind",
    "GoalRuntime",
    "GoalStateComponent",
    "GoalStatus",
    "HomeostasisComponent",
    "HomeostasisConfiguration",
    "Footprint",
    "LegacyCellCoordinate",
    "MicrocellCoordinate",
    "InformationNamespaceComponent",
    "InteractionCountCriterion",
    "InteractionExecutionComponent",
    "InteractionFailureReason",
    "InteractionRequestComponent",
    "InteractionSpecification",
    "InteractionType",
    "InteractionVerb",
    "MemoryComponent",
    "LineageIdGenerator",
    "MovementComponent",
    "MovementObstruction",
    "NavigationComponent",
    "NavigationPrimitive",
    "NavigationPrimitiveKind",
    "NavigationStatus",
    "NpcComponent",
    "OccupancySlot",
    "OccupancySlotsComponent",
    "OpenableComponent",
    "ObjectDimensions",
    "ObjectEffect",
    "ObjectIntrinsicComponent",
    "ObjectSizeClass",
    "OwnershipComponent",
    "KnowledgeRecord",
    "KnownTextAddressesComponent",
    "PendingSpeechComponent",
    "PendingTextReceiptsComponent",
    "PendingEngagementComponent",
    "PerceptionComponent",
    "PhysicalObjectIdentityComponent",
    "PhysicalInteractionRegistry",
    "PhysicalInteractionTarget",
    "PhysicalPose",
    "PhysicalRelationKind",
    "PhysicalStateComponent",
    "PlanAction",
    "PlanComponent",
    "PositionComponent",
    "PossessionsComponent",
    "PortableComponent",
    "PossessionThresholdCriterion",
    "SensesComponent",
    "SpatialCollisionError",
    "SpatialIndex",
    "SpatialIndexEntry",
    "SpatialLocationComponent",
    "SpatialMetric",
    "STANDING_CHARACTER_FOOTPRINT",
    "SpatialParentRelationComponent",
    "SimulationTimeCriterion",
    "StateComparisonCriterion",
    "System1Configuration",
    "System1State",
    "SupportComponent",
    "TextActionExecutionComponent",
    "TextActionRequestComponent",
    "TextContentPersistenceBinding",
    "TravelComponent",
    "TransactionExecutionComponent",
    "TransactionRequestComponent",
    "ReadableComponent",
    "RejectedEngagementGroup",
    "ScentSourceComponent",
    "SenseEffectTarget",
    "SenseModality",
    "SenseTransmission",
    "UsableComponent",
    "WearableComponent",
    "VisionObstruction",
    "footprints_touch",
    "validate_spatial_relation_acyclicity",
    "CardinalOrientation",
    "ActionOutcome",
    "ActionOutcomeCriterion",
    "LocationMatchCriterion",
    "default_activity_rates",
    "default_drive_thresholds",
]
