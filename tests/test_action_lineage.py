            ...
            "entities": [
                {
                    ...
                    "components": {
                        ...
                        "planner": {
                            "daily_goals": ["Reach the destination"],
                            "goals": [
                                {
                                    "id": "goal-destination",
...
def test_scenario_actions_receive_stable_ids_and_preserve_legacy_json() -> None:
    scenario = load_scenario(ROOT / "scenarios" / "system1-preemption.json")
...
def test_planner_and_system1_origins_and_preemption_are_explicit() -> None:
    planning = create_runner(
        load_scenario(ROOT / "scenarios" / "fake-llm-planning.json"),
    ...
    planner_action = next(
        ...
        if event.event_type == "action.queued"
        and event.payload["action_origin"] == "planner"
    )
    assert planner_action.payload["plan_id"] == "plan-00000001"
    assert planner_action.payload["plan_revision"] == 1

    survival = create_runner(
        load_scenario(ROOT / "scenarios" / "system1-preemption.json"),
        run_id="system1-lineage",
    )
    survival.run_for(1)
    events = survival.events.events
    interrupted = next(
        event for event in events if event.event_type == "action.interrupted"
    )
    cancelled = next(
    ...
    assert interrupted.payload["reason"] == "system1_preemption"
    assert cancelled.payload["reason"] == "system1_preemption"
...
def test_failed_action_and_plan_clearing_do_not_create_success() -> None:
    ...
    source["entities"][0]["components"].pop("planner")
