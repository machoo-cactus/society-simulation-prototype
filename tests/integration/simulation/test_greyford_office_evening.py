...
SCENARIO_PATH = EXAMPLE_SCENARIOS / "greyford-office-evening.json"
...
def test_greyford_materializes_city_neighborhood_and_character_knowledge() -> None:
    ...
    profile = FileSystemCharacterLibrary(EXAMPLE_CHARACTERS).get(CHARACTER_ID)
    ...
    assert any(
        goal["description"] == "take the metro home"
        and goal["priority"] == 100
        for goal in goals
    )
