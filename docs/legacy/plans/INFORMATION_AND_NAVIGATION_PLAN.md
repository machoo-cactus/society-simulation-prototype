...
> Archived proposal and migration record. Its implemented architecture is
> summarized in `docs/ARCHITECTURE.md`; unfinished work is tracked in
> `docs/roadmaps/INFORMATION_AND_NAVIGATION.md`.
...
character-known topology projection, and canonical `NAVIGATE` execution path
are implemented. `navigate_to` is the only controller navigation tool.
`MovementComponent` and `TravelComponent` remain deterministic execution
primitives beneath navigation.
...
Controller navigation uses only `navigate_to`.
...
4. Expose only `navigate_to` at the controller boundary.
