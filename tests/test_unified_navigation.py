...
from stage0_sim.domain.intents import MoveIntent, NavigationIntent, TravelIntent
...
def test_navigate_tool_and_compatibility_aliases_produce_typed_intents() -> None:
    targets = (
    ...
    go_to = tools.propose(
        request,
        ModelToolCall("call-go", "go_to", {"target_id": "sofa"}),
    )
    travel_to = tools.propose(
        request,
        ModelToolCall(
            "call-travel",
            "travel_to",
    ...
    assert isinstance(go_to, MoveIntent)
    assert isinstance(travel_to, TravelIntent)
...
def test_navigate_tool_commits_one_general_navigation_intention() -> None:
    ...
    started = [
        ...
        if event.event_type == "plan.action_started"
    ]
...
def test_travel_to_alias_commits_navigation_instead_of_a_parallel_architecture() -> None:
    payload = _city_payload_without_plan()
    payload["cognition"]["controller"] = "tool-agent"
    components = payload["entities"][0]["components"]
    components["controller"] = {
        "enabled": True,
        "tool_allowlist": ["travel_to"],
    }
    components["information"] = {
        "documents": [
            {
...
def test_navigation_propagates_vehicle_unavailable_failure() -> None:
    ...
    assert failures == {
        ...
        "plan.action_failed": "vehicle_not_available",
    }
